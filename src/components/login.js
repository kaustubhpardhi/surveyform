...
const Login = () => {
  ...
  const submitHandler = async (event) => {
    ...
    try {
      const response = await axios.post("/surveyor/surveyorlogin", {
        surveyorId: id,
        surveyorPass: password,
      });

      const { surveyorId } = response.data;
      localStorage.setItem("user", JSON.stringify({ id: surveyorId }));
      setChange(!change);
      setCookie("user", JSON.stringify({ id: surveyorId }), true);
      navigate("/form");
    } catch (error) {
      if (error.response) {
